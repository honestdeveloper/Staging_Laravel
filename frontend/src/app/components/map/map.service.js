    ...
    angular
                    ...
                    )
            {
                ...
                function _loadCategories(data) {
                    $rootScope.searchSpotCategories = {};
                    _.each(data, function (item) {
                        $rootScope.searchSpotCategories[item.name] = item.categories;
                    });
                }
                ...
                //save selection
                function SaveSelections(title, description, callback) {
                    ClearSelectionListeners();
                    if (pathSelectionStarted) {
                        CancelPathSelection();
                    }
                ...
                function getFiltersData()
                {
                    ...
                    if ($rootScope.mapSearchType) {
                        data.searchType = $rootScope.mapSearchType;
                    }
                    if ($rootScope.filterOptions.minRating) {
                        data.filter.rating = $rootScope.filterOptions.minRating;
                    }
                    if ($rootScope.filterOptions.maxPrice) {
                        data.filter.price = $rootScope.filterOptions.maxPrice;
                    }
                    ...
                    if ($rootScope.filterOptions.isApproved) {
                        data.filter.is_approved = $rootScope.filterOptions.isApproved;
                    }
                    if ($rootScope.filterOptions.tags) {
                        data.filter.tags = _.pluck($rootScope.filterOptions.tags, 'text');
                    }
                    if ($rootScope.filterOptions.dateFrom) {
                        if ($rootScope.filterOptions.dateFrom.match(/^[0-9]{1,2}\.[0-9]{2}\.[0-9]{4}$/)) {
                            data.filter.start_date = moment($rootScope.filterOptions.dateFrom, DATE_FORMAT.datepicker.date).format(DATE_FORMAT.backend_date);
                        } else if ($rootScope.filterOptions.dateFrom.match(/^[0-9]{4}\-[0-9]{1,2}\-[0-9]{1,2}$/)) {
                            data.filter.start_date = $rootScope.filterOptions.dateFrom;
                        }
                    }
                    if ($rootScope.filterOptions.dateTo) {
                        if ($rootScope.filterOptions.dateTo.match(/^[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{4}$/)) {
                            data.filter.end_date = moment($rootScope.filterOptions.dateTo, DATE_FORMAT.datepicker.date).format(DATE_FORMAT.backend_date);
                        } else if ($rootScope.filterOptions.dateTo.match(/^[0-9]{4}\-[0-9]{1,2}\-[0-9]{1,2}$/)) {
                            data.filter.end_date = $rootScope.filterOptions.dateTo;
                        }
                    }
                    var categories = _.where($rootScope.searchSpotCategories[$rootScope.sortLayer], {selected: true});
                    if (categories.length > 0) {
                ...
                function setFiltersData(data)
                {
                    ...
                    $rootScope.mapSelectionProps = {};
                    if (data.searchType) {
                        $rootScope.mapSearchType = data.searchType;
                    }
                    if (data.filter.rating) {
                        $rootScope.filterOptions.minRating = data.filter.rating;
                    }
                    if (data.filter.price) {
                        $rootScope.filterOptions.maxPrice = data.filter.price;
                    }
                    if ($rootScope.mapSearchType && $rootScope.mapSearchType === 'radius')
                    {
                        $rootScope.mapSelectionProps.lat = data.lat;
                        $rootScope.mapSelectionProps.lng = data.lng;
                        $rootScope.mapSelectionProps.radius = data.radius;
                    }
                    if ($rootScope.mapSearchType && $rootScope.mapSearchType === 'lasso')
                    {
                        $rootScope.mapSelectionProps.vertices = data.vertices;
                    }
                    if (data.filter.is_approved) {
                        $rootScope.filterOptions.isApproved = data.filter.is_approved;
                    }
                    if (data.filter.tags) {
                        _.each(data.filter.tags, function (value, index) {
                            $rootScope.filterOptions.tags.push({text: value});
                        });
                    }
                    ...
                    if (data.filter.category_ids)
                    {
                        $rootScope.filterOptions.category = data.filter.category_ids[0];
                    }
                }
